...
private:
    ...
    static int print_bottom_line() {
        
        char choose_menu;

        cout << "\nPress 'm' to Menu or any other key to Exit: " << endl;;
        cin >> choose_menu;
        if (choose_menu == 'm' || choose_menu == 'M') {
...
public:
    ...
    static void ConnectionFunction(
        ...
        const char*  pwd, const char*  database) {
        ...
        conn = mysql_real_connect \
            (conn, server, user, pwd, database, 0, NULL, 0);
        if (conn)
        {
